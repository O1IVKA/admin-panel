(function($) {
	...
	/*----------------------------------------------------*/
		...
		$('.nav-item').each((index, elem) => {
			let link = $(elem).find('a').attr('href').split('?')[0].split(/[/#]/)[1];
			let locationHref = location.split('/')[3].split(/[#?]/)[0];
			if(locationHref == link){
	...
	var defineOrientation = (currentImage) => {
